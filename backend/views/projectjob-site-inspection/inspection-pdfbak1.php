...
<tbody>
  <?php foreach ($pissTask as $key => $value): ?>
    ...
    <tr>
      ...
      <td style="width:20%" class="ipi-task-data"><?php echo $value->description ?></td>
      <td style="width:5%;text-align:center;" class="ipi-task-data">
        <?php if ($value->status == 'YES') {
          echo '&#10004;';
        } ?>
      </td class="ipi-task-data">
      ...
      <td style="width:5%;text-align:center;" class="ipi-task-data">
        <?php if ($value->status != 'YES' && $value->status != 'NO') {
          echo '&#10004;';
        } ?>
      </td>
      <td style="width:20%" class="ipi-task-data"><?php echo $value->non_conformance ?></td>
      <td style="width:20%" class="ipi-task-data"><?php echo $value->corrective_actions ?></td>
      <td style="width:20% text-align:center;" class="ipi-task-data"><?php echo $value->target_completion_date ?></td>
    </tr>
      <?php $i+=1 ?>
  <?php endforeach; ?>
...
<div class="ipi-corrective-container">
  ...
  <table class="ipi-corrective" border=1>
    ...
    <tbody>
      <?php foreach ($correct as $key => $value): ?>
        <tr>
          <td style="width:5%;text-align:center;" class="correct-data"><?php echo $i ?></td>
          <td style="width:10%;text-align:center;" class="correct-data"><?php echo $value->car_no ?></td>
          <td style="width:20%" class="correct-data"><?php echo $value->description ?></td>
          <td style="width:15%;text-align:center;" class="correct-data"><?php echo $value->target_remedy_date ?></td>
          <td style="width:15%;text-align:center;" class="correct-data"><?php echo $value->completion_date ?></td>
          <td style="width:20%" class="correct-data"><?php echo $value->remarks ?></td>
          <td style="width:15%" class="correct-data"><?php echo $value->disposition ?> </td>
        </tr>
        <?php $i+=1 ?>
      <?php endforeach; ?>
